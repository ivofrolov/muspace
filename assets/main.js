...
{
	if (region.a8.az === region.bq.az)
	{
		return 'on line ' + region.a8.az;
	}
	return 'on lines ' + region.a8.az + ' through ' + region.bq.az;
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {bu: fragment, bv: host, bE: path, bG: port_, bJ: protocol, bK: query};
	});
...
var $author$project$Main$Resize = $elm$core$Basics$identity;
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		...
		A2($elm$core$Task$perform, $elm$core$Basics$identity, $elm$browser$Browser$Dom$getViewport));
};
var $elm$core$Platform$Sub$batch = _Platform_batch;
var $elm$core$Platform$Sub$none = $elm$core$Platform$Sub$batch(_List_Nil);
var $author$project$Main$fitGrid = F3(
	function (width, height, cellSize) {
		return {Y: cellSize, J: ((width - ((width / cellSize) | 0)) / cellSize) | 0, aD: ((height - ((height / cellSize) | 0)) / cellSize) | 0};
	});
	...
	function (msg, model) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					ax: A3($author$project$Main$fitGrid, viewport.bW.cv | 0, viewport.bW.b9 | 0, 26)
				}),
		...
		switch (declaration.$) {
			...
			case 6:
				...
				return $elm$core$String$isEmpty(record.b4) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
					A3($elm$core$Dict$insert, record.ci, record.b4, keyframesByName),
					declarations);
			case 7:
				var properties = declaration.a;
				return $elm$core$List$isEmpty(properties) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
...
var $rtfeldman$elm_css$Css$Structure$compactStylesheet = function (_v0) {
	...
	return {
		bn: charset,
		b5: $rtfeldman$elm_css$Css$Structure$compactDeclarations(declarations),
		bw: imports,
		bB: namespaces
	};
};
...
var $robinheghan$murmur3$Murmur3$HashData = F4(
	function (shift, seed, hash, charsProcessed) {
		return {Z: charsProcessed, ad: hash, R: seed, aj: shift};
	});
...
var $robinheghan$murmur3$Murmur3$hashFold = F2(
	function (c, data) {
		...
		if (_v0 === 24) {
			...
			return {Z: data.Z + 1, ad: res, R: data.R, aj: data.aj + 8};
		}
	});
...
var $rtfeldman$elm_css$Css$Preprocess$Resolve$toStructure = function (_v0) {
	...
	var declarations = $rtfeldman$elm_css$Css$Preprocess$Resolve$extract(
		A2($elm$core$List$concatMap, $rtfeldman$elm_css$Css$Preprocess$unwrapSnippet, snippets));
	return {bn: charset, b5: declarations, bw: imports, bB: namespaces};
};
...
var $rtfeldman$elm_css$Css$Preprocess$stylesheet = function (snippets) {
	return {bn: $elm$core$Maybe$Nothing, bw: _List_Nil, bB: _List_Nil, bP: snippets};
};
...
var $author$project$Main$note = F2(
	function (i, j) {
		var steps = A2($elm$core$Basics$modBy, 12, (i * (12 - 4)) + (j * 7));
...
var $author$project$Main$generateSpace = function (grid) {
	var cell = F2(
		function (i, j) {
			return A3(
				...
				i,
				j,
				A2($author$project$Main$note, i, j));
		});
	return A2(
		$elm$core$List$map,
		function (x) {
			return A2(
				cell,
				(x / grid.J) | 0,
				A2($elm$core$Basics$modBy, grid.J, x));
		},
		A2($elm$core$List$range, 0, (grid.aD * grid.J) - 1));
};
...
var $rtfeldman$elm_css$Css$monospace = {K: 0, z: 'monospace'};
	...
	return $rtfeldman$elm_css$Css$batch(
		_List_fromArray(
			[
				...
				'--columns',
				$elm$core$String$fromInt(grid.J)),
				A2(
				$rtfeldman$elm_css$Css$property,
				'--rows',
				$elm$core$String$fromInt(grid.aD)),
				A2(
				$rtfeldman$elm_css$Css$property,
				'--cell-size',
				$elm$core$String$fromInt(grid.Y) + 'px'),
				...
				A2($rtfeldman$elm_css$Css$property, 'column-gap', '1px'),
				$rtfeldman$elm_css$Css$fontFamily($rtfeldman$elm_css$Css$monospace),
				$rtfeldman$elm_css$Css$fontSize(
				$rtfeldman$elm_css$Css$px(grid.Y / 2)),
				$rtfeldman$elm_css$Css$textAlign($rtfeldman$elm_css$Css$center),
				$rtfeldman$elm_css$Css$lineHeight(
				$rtfeldman$elm_css$Css$px(grid.Y)),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(cell.bS)
					]))
			]));
};
				...
				A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$author$project$Main$gridStyle(model.ax)
								]))
						]),
					A2(
						$elm$core$List$map,
						$author$project$Main$renderCell,
						$author$project$Main$generateSpace(model.ax))))
			]),
		cr: 'muspace'
	};
};
