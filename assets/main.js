...
{
	if (region.bb.aA === region.bw.aA)
	{
		return 'on line ' + region.bb.aA;
	}
	return 'on lines ' + region.bb.aA + ' through ' + region.bw.aA;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { cd: 'hidden', b6: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { cd: 'mozHidden', b6: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { cd: 'msHidden', b6: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { cd: 'webkitHidden', b6: 'webkitvisibilitychange' }
		: { cd: 'hidden', b6: 'visibilitychange' };
}
...
var $author$project$Main$fitGrid = F3(
	function (width, height, grid) {
		return _Utils_update(
			grid,
			{L: ((width + grid.P) / (grid.K + grid.P)) | 0, aE: ((height + grid.P) / (grid.K + grid.P)) | 0});
	});
...
var $author$project$Main$symmetricDifferenceSets = F2(
	function (set, other) {
		return A3(
			$elm$core$Set$foldl,
			F2(
				function (x, xs) {
					return A2($elm$core$Set$member, x, xs) ? A2($elm$core$Set$remove, x, xs) : A2($elm$core$Set$insert, x, xs);
				}),
			other,
			set);
	});
		...
		if (!msg.$) {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						x: A3($author$project$Main$fitGrid, viewport.b$.cw | 0, viewport.b$.cc | 0, model.x)
					}),
			...
			return _Utils_Tuple2(
				_Utils_update(
					...
					{
						aF: A2(
							$author$project$Main$symmetricDifferenceSets,
							$elm$core$Set$singleton(position),
							model.aF)
					}),
				$elm$core$Platform$Cmd$none);
		}
	});
var $author$project$Main$ToggleNote = function (a) {
	return {$: 1, a: a};
};
		...
		switch (declaration.$) {
			...
			case 6:
				...
				return $elm$core$String$isEmpty(record.b7) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
					A3($elm$core$Dict$insert, record.cj, record.b7, keyframesByName),
					declarations);
			case 7:
				var properties = declaration.a;
				return $elm$core$List$isEmpty(properties) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
...
var $rtfeldman$elm_css$Css$Structure$compactStylesheet = function (_v0) {
	...
	var declarations = _v0.b8;
	return {
		bt: charset,
		b8: $rtfeldman$elm_css$Css$Structure$compactDeclarations(declarations),
		bC: imports,
		bH: namespaces
	};
};
...
var $rtfeldman$elm_css$Css$Preprocess$Resolve$toStructure = function (_v0) {
	...
	var declarations = $rtfeldman$elm_css$Css$Preprocess$Resolve$extract(
		A2($elm$core$List$concatMap, $rtfeldman$elm_css$Css$Preprocess$unwrapSnippet, snippets));
	return {bt: charset, b8: declarations, bC: imports, bH: namespaces};
};
...
var $rtfeldman$elm_css$Css$Preprocess$stylesheet = function (snippets) {
	return {bt: $elm$core$Maybe$Nothing, bC: _List_Nil, bH: _List_Nil, bV: snippets};
};
...
var $author$project$Main$generateCells = function (model) {
		...
		function (x, y) {
			return {
				a4: A2(
					...
					_Utils_Tuple2(x, y),
					model.aF),
				bd: A2($author$project$Main$noteNameAt, x, y),
				bf: x,
				bg: y
			};
		});
	return A2(
		...
		function (x) {
			return A2(
				cell,
				A2($elm$core$Basics$modBy, model.x.L, x),
				(x / model.x.L) | 0);
		},
		A2($elm$core$List$range, 0, (model.x.aE * model.x.L) - 1));
};
...
var $author$project$Main$gridStyle = function (grid) {
	return $rtfeldman$elm_css$Css$batch(
		_List_fromArray(
			[
				...
				'--columns',
				$elm$core$String$fromInt(grid.L)),
				A2(
				$rtfeldman$elm_css$Css$property,
				'--rows',
				$elm$core$String$fromInt(grid.aE)),
				A2(
				$rtfeldman$elm_css$Css$property,
				'--cell-size',
				$elm$core$String$fromInt(grid.K) + 'px'),
				A2(
				$rtfeldman$elm_css$Css$property,
				'--gap',
				$elm$core$String$fromInt(grid.P) + 'px'),
				A2($rtfeldman$elm_css$Css$property, 'display', 'grid'),
				A2($rtfeldman$elm_css$Css$property, 'grid-template-columns', 'repeat(var(--columns), var(--cell-size))'),
				A2($rtfeldman$elm_css$Css$property, 'grid-template-rows', 'repeat(var(--rows), var(--cell-size))'),
				A2($rtfeldman$elm_css$Css$property, 'row-gap', 'var(--gap)'),
				A2($rtfeldman$elm_css$Css$property, 'column-gap', 'var(--gap)'),
				$rtfeldman$elm_css$Css$fontFamily($rtfeldman$elm_css$Css$monospace),
				$rtfeldman$elm_css$Css$fontSize(
				$rtfeldman$elm_css$Css$px(grid.K / 2)),
				$rtfeldman$elm_css$Css$textAlign($rtfeldman$elm_css$Css$center),
				$rtfeldman$elm_css$Css$lineHeight(
				$rtfeldman$elm_css$Css$px(grid.K)),
...
var $rtfeldman$elm_css$Css$borderColor = function (c) {
	return A2($rtfeldman$elm_css$Css$property, 'border-color', c.A);
};
...
var $author$project$Main$cellStyle = $rtfeldman$elm_css$Css$batch(
	_List_fromArray(
		[
			A2($rtfeldman$elm_css$Css$property, 'border-width', 'var(--gap)'),
			$rtfeldman$elm_css$Css$borderStyle($rtfeldman$elm_css$Css$solid),
			$rtfeldman$elm_css$Css$borderColor(
			$rtfeldman$elm_css$Css$hex('#D2D2D2')),
			...
			_List_fromArray(
				[
					...
					$rtfeldman$elm_css$Css$hex('#D5D5D5'))
				]))
		]));
var $author$project$Main$highlightedCellStyle = $rtfeldman$elm_css$Css$batch(
	_List_fromArray(
		[
			$author$project$Main$cellStyle,
			$rtfeldman$elm_css$Css$backgroundColor(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				'data-x',
				$elm$core$String$fromInt(cell.bf)),
				A2(
				$rtfeldman$elm_css$Html$Styled$Attributes$attribute,
				'data-y',
				$elm$core$String$fromInt(cell.bg)),
				$rtfeldman$elm_css$Html$Styled$Attributes$css(
				_List_fromArray(
					[
						cell.a4 ? $author$project$Main$highlightedCellStyle : $author$project$Main$cellStyle
					]))
			]),
		_List_fromArray(
			[
				$rtfeldman$elm_css$Html$Styled$text(cell.bd)
			]));
};
						...
						[
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$author$project$Main$gridStyle(model.x)
										])),
									$author$project$Main$onCellClick($author$project$Main$ToggleNote)
								]),
							A2(
								$elm$core$List$map,
